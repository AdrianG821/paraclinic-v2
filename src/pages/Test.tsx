...
import { Trash2 , NotebookPen , Check , X , ArrowDownRight, ArrowUpRight} from "lucide-react";
        ...
        const uniqueRefs = Array.from(
            new Map(
                reqInvList.filter(r => r.ref_id !== null && r.ref.name !== null ).map(w => [
                    w.ref_id,
                    {investigation_id: w.investigation_id , ref: w.ref.name ?? "" , result: w.result, normal_values: {min: w.normal_values?.min , max: w.normal_values?.max}}
                ] as const)
                .filter(([, v]) => v.ref !== "")
            ).values())
    ...
    return (
                    ...
                    {uniqueRef.map((u) => 
                        (u.investigation_id === tx.investigation_id ? (
                            <tr key={`ref-${tx.id}-${u.ref}`} className="bg-transparent hover:bg-slate-900/40">
                                ...
                                <td className="p-2 w-1/6 truncate border-0">{u.normal_values?.min} - {u.normal_values?.max}</td> 
                              : <td></td>}           
                            </tr>
                        ) : null
                        ))}
        ...
        {addInvestigations && (
            ...
            >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button className="p-2 rounded-full bg-blue-400 border-slate-700 hover:bg-white" type="submit"  onClick={insertInv}>
                    Inchide X
                    </button>
                </div>
        ...
        {aModify && (
            ...
            >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h3 style={{ margin: 0 }}>Adauga investigatii</h3>
                    <button className="p-2 rounded-full bg-blue-400 border-slate-700 hover:bg-white" type="submit"  onClick={() => (setAModify(false))}>
                    Inchide X
                    </button>
                </div>
                {
                    resultModify.map((tx) => (
                        <div key={tx.id} className="p-2 flex flex-col">
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <textarea key={tx.id} className="border border-slate-800 w-2/5 h-48 max-h-96" value={result[tx.id] ?? ""}  onChange={(e) => setResult(prev => ({...prev, [tx.id]: e.target.value}))} />

                                : <input key={tx.id} className="border border-slate-800 w-24 "  value={result[tx.id]} onChange={(e) => setResult(prev => ({...prev, [tx.id]: e.target.value}))} />}
                                <button className="p-2 rounded-full bg-blue-400 border-slate-700 hover:bg-white ml-14" onClick={() => handleSaveResults(tx.investigation_id, tx.id , tx.investigation.machinesId , result[tx.id])}>Salveaza</button>
                            </div>
                        </div>
                    ))
                }
        ...
        {patientLoaded && <button onClick={handleCreateRequest} className="px-6 py-2.5 rounded-xl bg-emerald-600 text-white font-medium hover:bg-emerald-500 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30">Salveaza</button>}
        </div>
        </div>
    )
}